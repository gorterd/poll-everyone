class PresentationChannel < ApplicationCable::Channel
  ...
  def respond(response)
    ...
    @response = @answer_option.responses.build( participant_id: response['participantId'] )

    if @response.save
      PresentationChannel.broadcast_to(@presenter, type: 'RESPONSE', data: response_json)
    end
  end
  ...
  def clear(response)
    ...
    if @response.destroy
      PresentationChannel.broadcast_to(@presenter, type: 'CLEAR_RESPONSE', data: response_json)
    end
  end
